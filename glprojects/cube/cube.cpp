...
*/
	...
	glr.shader(
		...
		"void main( void ) {\n"
			...
			"fColor = vec4( clamp( pow( a / dot( d, d ), 2. ), .1, 1. ) * vec3( .987,.654,.321 ), 1.f );\n"
		"}\n",
		GLRenderer::ShaderCode::FROM_CODE ).
		addUniform( "modelView", GLRenderer::Shader::MAT4, GLRenderer::Shader::SCALAR, & modelView ).
